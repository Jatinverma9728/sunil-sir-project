            ...
            <div className="max-w-[1600px] mx-auto px-4 md:px-6 lg:px-8">
                ...
                <div className="flex items-center justify-between mb-8">
                    <div className="flex items-center gap-6">
                        ...
                        {/* Countdown Timer */}
                        {activeOffer && (
                            <div className="flex items-center gap-3 bg-white px-4 py-2 rounded-full shadow-sm border border-rose-100">
                                <span className="text-sm font-semibold text-gray-500 uppercase tracking-wide">Ends in:</span>
                                <div className="flex gap-1">
                                    {[
                                        ...
                                        <div key={index} className="flex items-center">
                                            <div className="bg-gradient-to-br from-rose-500 to-orange-500 text-white w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold shadow-sm">
                                                {String(time.value).padStart(2, "0")}
                                            </div>
                                            {index < 2 && <span className="text-gray-400 mx-1 font-bold">:</span>}
                                        </div>
                                    ))}
                                </div>
                            </div>
                    ...
                    >
                        See All
                        <span className="w-8 h-8 rounded-full bg-rose-100 flex items-center justify-center group-hover:bg-rose-600 group-hover:text-white transition-all duration-300">
                ...
                {loading ? (
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
                        {[...Array(5)].map((_, i) => (
                            <div key={i} className="bg-white rounded-[2rem] p-4 animate-pulse h-[300px]"></div>
                        ))}
                    </div>
                ) : (
                    /* Products Grid */
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
                        {products.map((product) => (
                            <Link
                                ...
                                className="group bg-white border border-rose-100 rounded-[2rem] overflow-hidden hover:shadow-[0_20px_40px_-15px_rgba(244,63,94,0.1)] transition-all duration-500 hover:-translate-y-1"
                            >
                                {/* Product Image */}
                                <div className="relative aspect-square bg-rose-50/30 p-6">
                                    {product.image ? (
                                        <Image
                                            src={product.image}
                                            alt={product.title}
                                            fill
                                            className="object-contain p-4 group-hover:scale-110 transition-transform duration-500 mixing-blend-multiply"
                                        />
                                    ) : (
                                    ...
                                    {product.discount > 0 && (
                                        <span className="absolute top-4 left-4 bg-rose-500 text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-sm">
                                            -{product.discount}%
                                        </span>
                                    )}
                                </div>
                                ...
                                {/* Product Info */}
                                <div className="p-5">
                                    <h3 className="text-sm font-semibold text-gray-900 mb-3 line-clamp-2 min-h-[40px] group-hover:text-rose-600 transition-colors">
                                        {product.title}
                                    </h3>
                                    ...
                                    <div className="flex items-center justify-between text-xs pt-3 border-t border-gray-50">
                                        ...
                                        <span className="text-gray-400 font-medium">{product.soldCount} claimed</span>
                                    </div>
