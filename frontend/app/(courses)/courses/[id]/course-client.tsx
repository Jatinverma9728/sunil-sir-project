...
export default function CourseDetailClient() {
    ...
    const fetchCourse = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setCourse(data.success && data.data ? data.data : null);
            } else {
                setCourse(null);
            }
        } catch (error) {
    ...
    const handleEnroll = () => {
        ...
        if (course.isPurchased) {
            ...
            router.push(lessons.length > 0 ? `/courses/${courseId}/lessons/${lessons[0]._id}` : `/my-courses`);
            return;
        }
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-gray-50">
                <div className="text-center">
                    ...
                    <Link href="/courses" className="inline-flex items-center gap-2 px-6 py-3 bg-slate-900 text-white font-medium rounded-lg hover:bg-slate-800 transition-colors">
                        Browse Courses
                    </Link>
                </div>
            </div>
                ...
                <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 py-12 lg:py-16 relative">
                    <div className="grid lg:grid-cols-3 gap-8 lg:gap-12">
                        <div className="lg:col-span-2">
                            ...
                            <span className="inline-block px-3 py-1 bg-blue-500/20 text-blue-300 text-sm font-medium rounded-full mb-4 capitalize">{course.level}</span>
                            <h1 className="text-3xl lg:text-4xl xl:text-5xl font-bold mb-4 leading-tight">{course.title}</h1>
                            <p className="text-lg text-slate-300 mb-6 leading-relaxed max-w-2xl">{course.description.split('\n')[0]}</p>

                            <div className="flex flex-wrap items-center gap-4 lg:gap-6 mb-6">
                                <div className="flex items-center gap-2">
                                    <div className="flex">
                                        {[1, 2, 3, 4, 5].map((star) => (
                                            <svg key={star} className={`w-5 h-5 ${star <= Math.floor(getRating()) ? 'text-amber-400' : 'text-slate-600'}`} fill="currentColor" viewBox="0 0 20 20">
                    ...
                    <div className="lg:col-span-2 space-y-8">
                        ...
                        <div className="bg-white rounded-xl border border-gray-200 p-6 lg:p-8">
                            <div className="flex items-center justify-between mb-6">
                                ...
                                <p className="text-sm text-gray-500">{course.syllabus?.length || 0} sections • {lessonCount} lectures</p>
                            </div>
                            <div className="space-y-3">
                                {(course.syllabus || []).map((section, index) => (
                                    <div key={index} className="border border-gray-200 rounded-lg overflow-hidden">
                                        ...
                                        >
                                            <div className="flex items-center gap-3">
                                                <svg className={`w-4 h-4 text-gray-500 transition-transform ${expandedSections.includes(index) ? 'rotate-90' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
                                        ...
                                        {expandedSections.includes(index) && (
                                            <div className="divide-y divide-gray-100">
                                                {section.lectures.map((lecture, li) => (
                                                    <div key={li} className="flex items-center justify-between px-4 py-3 pl-11">
                                                        <div className="flex items-center gap-3">
                        ...
                        <div className="bg-white rounded-xl border border-gray-200 p-6 lg:p-8">
                            ...
                            <div className="flex gap-6">
                                <div className="w-24 h-24 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full flex items-center justify-center flex-shrink-0">
                                    <span className="text-3xl text-white font-bold">{course.instructor?.name?.charAt(0) || 'I'}</span>
                                </div>
                                <div className="flex-1">
                                    <h3 className="text-xl font-bold text-gray-900 mb-1">{course.instructor?.name || 'Expert Instructor'}</h3>
                                    {course.instructor?.title && <p className="text-gray-500 mb-4">{course.instructor.title}</p>}
                    ...
                    <div className="lg:col-span-1">
                        <div className="lg:sticky lg:top-24">
                            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
                                <div className="relative aspect-video bg-slate-900 cursor-pointer group" onClick={() => setShowVideoModal(true)}>
                                    {course.thumbnail ? (
                                ...
                                <div className="p-6">
                                    <div className="mb-6">
                                        ...
                                        {discount > 0 && (
                                            <span className="inline-block px-2.5 py-1 bg-rose-100 text-rose-700 text-sm font-semibold rounded">{discount}% off</span>
                                        )}
                                    </div>
                                    ...
                                    <div className="space-y-3 mb-6">
                                        <button
                                            ...
                                            className={`w-full py-3.5 rounded-lg font-semibold transition-colors flex items-center justify-center gap-2 ${course.isPurchased ? 'bg-emerald-600 hover:bg-emerald-700 text-white' : 'bg-slate-900 hover:bg-slate-800 text-white'}`}
                                        >
                                            {course.isPurchased ? 'Go to Course' : course.price === 0 ? 'Enroll Free' : 'Enroll Now'}
                                        </button>
                                        {!course.isPurchased && (
                ...
                <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-4" onClick={() => setShowVideoModal(false)}>
                    <div className="max-w-4xl w-full" onClick={(e) => e.stopPropagation()}>
                        <div className="flex justify-end mb-4">
                            <button onClick={() => setShowVideoModal(false)} className="w-10 h-10 bg-white/10 hover:bg-white/20 rounded-full flex items-center justify-center text-white transition-colors">
