...
const courses = [
    {
        ...
        level: "Beginner",
        lessons: 12,
        color: "bg-gradient-to-br from-violet-50 to-indigo-50",
            ...
            <div className="max-w-[1600px] mx-auto px-4 md:px-6 lg:px-8">
                ...
                <div className="grid md:grid-cols-3 gap-8">
                    {courses.map((course) => (
                        ...
                        >
                            {/* Card Top */}
                            <div className={`relative h-56 p-8 flex flex-col justify-between ${course.color}`}>
                                <div className="flex justify-between items-start">
                                    <div className={`w-14 h-14 bg-white rounded-2xl flex items-center justify-center shadow-sm ${course.iconColor} group-hover:scale-110 transition-transform duration-300`}>
                                        <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
                                        </svg>
                                    </div>
                                    <span className="bg-white/60 backdrop-blur-sm px-3 py-1 rounded-full text-xs font-semibold text-gray-700">
                                        {course.level}
                                    </span>
                                </div>
                                <div>
                                    <span className="text-sm font-medium text-gray-500 mb-1 block">{course.lessons} Lessons</span>
                                    <h3 className="text-xl font-bold text-gray-900 leading-tight group-hover:text-indigo-700 transition-colors">
                                        {course.title}
                                    </h3>
                                </div>
                            </div>
